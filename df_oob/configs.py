    ...
    if expno_name != '000CR':
        ...
        for run_id in range(n_runs):
            ...
            for n_data_to_be_valued in [5000, 25000]:
                for input_dim in [20, 60, 100]:
                    for masked_ratio in [0.5, 0.8]:
...
def config000CR():
    exp, runs=generate_config(expno_name='000CR', problem='clf', dataset='gaussian', n_runs=5)
    return exp, runs  
...
def config005CR():
    exp, runs=generate_config(expno_name='005CR', problem='clf', dataset='vehicle_sensIT')
    return exp, runs  

def config006CR():
    exp, runs=generate_config(expno_name='006CR', problem='clf', dataset='electricity')
    return exp, runs  
...
def config011CR():
    exp, runs=generate_config(expno_name='011CR', problem='clf', dataset='webdata_wXa')
    return exp, runs 

...
class FeatureApproach(object):
    ...
    def _calculate_proposed_data_oob(self):
        ...
        # self.data_value_dict['Data-OOB-var']=(1-self.rf_model_original.evaluate_oob_accuracy(self.X, self.y)).to_numpy()
    ...
    def _calculate_proposed_df_oob(self):
        ...
        time_init=time()
        df_oob_series = self.rf_model.evaluate_dfoob_accuracy(self.X, self.y, adjust = True, acc = True)
        ...
        # time_init=time()
        # df_oob_series_original = self.rf_model_original.evaluate_dfoob_accuracy(self.X, self.y, adjust = True, acc = True)
        # self.df_value_dict['Df-OOB-original'] = df_oob_series_original
        # df_oob_data_original, df_oob_feature_original = df_oob_agg(df_oob_series_original)
        # self.data_value_dict['Df-OOB-data-original']=df_oob_data_original.to_numpy()
        # self.feature_value_dict['Df-OOB-feature-original']=df_oob_feature_original.to_numpy()
        # self.time_dict['Df-OOB-original']=time()-time_init
        
        # df_oob_series_noacc = self.rf_model.evaluate_dfoob_accuracy(self.X, self.y, adjust = True, acc = False)
        # df_oob_series_unadj = self.rf_model.evaluate_dfoob_accuracy(self.X, self.y, adjust = False)
        # self.df_value_dict['Df-OOB_noacc'] = df_oob_series_noacc
        # self.df_value_dict['Df-OOB-unadj'] = df_oob_series_unadj
        # df_oob_data_noacc, df_oob_feature_noacc = df_oob_agg(df_oob_series_noacc)
        # df_oob_data_unadj, df_oob_feature_unadj = df_oob_agg(df_oob_series_unadj)
        # self.data_value_dict['Df-OOB-data-noacc']=df_oob_data_noacc.to_numpy()
        # self.feature_value_dict['Df-OOB-feature-noacc']=df_oob_feature_noacc.to_numpy()

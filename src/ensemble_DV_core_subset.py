...
def _parallel_build_trees(
    ...
    if bootstrap:
        ...
        tree.fit(X[:,features_used], y, sample_weight=curr_sample_weight, check_input=False) 
...
class RandomForestClassifierDV_subset(RandomForestClassifier):
    ...
    def evaluate_oob_accuracy(self, X, y):
        ...
        oob_performance=[]
        ensemble_features_index = [np.where(i == 1)[0] for i in self._ensemble_features]
    ...
    def evaluate_dfoob_accuracy(self, X, y):
        ...
        for key in dfoob:
            dfoob[key] = dfoob[key][0]/dfoob[key][1] if dfoob[key][1]!=0 else 0

            
        return pd.Series(dfoob)

    def evaluate_dfoob_accuracy_distance(self, X, y, weight=0, abl=False):
        import pandas as pd
        assert len(self.estimators_)!=0, 'Run fit first. self.estimators_ is not defined'
        assert len(self._ensemble_X)!=0, 'Run evaluate_importance first. self._ensemble_X is not defined'
        ...
        for i, weak_learner in enumerate(self.estimators_):
            oob_ind=np.where(self._ensemble_X[i] == 0)[0]
            in_features = in_features_index[i]
            
            oob_acc=(weak_learner.predict(X[oob_ind][:,in_features])==y[oob_ind]).astype(float)
            
            bootstrap_indices = np.repeat(np.arange(n), self._ensemble_X[i].astype(int))
            bootstrap_samples = X[bootstrap_indices][:, in_features]
            bootstrap_y = y[bootstrap_indices]
            class_0_mean = np.mean(bootstrap_samples[bootstrap_y == 0], axis=0)
            class_1_mean = np.mean(bootstrap_samples[bootstrap_y == 1], axis=0)
            means_matrix = np.where(bootstrap_y[:, np.newaxis] == 0, class_0_mean, class_1_mean)

            distances = np.linalg.norm(bootstrap_samples - means_matrix, axis=1)
            max_distance = np.max(distances)
            min_distance = np.min(distances)

            for ind, j in enumerate(oob_acc):
                current_sample = X[oob_ind[ind], in_features]
                if y[oob_ind[ind]] == 0:
                    mean_vector = class_0_mean
                else:
                    mean_vector = class_1_mean
                l2_distance = -np.linalg.norm(current_sample - mean_vector)
                normalized_distance = (l2_distance - min_distance) / (max_distance - min_distance) if max_distance != min_distance else 0
                for feature_ind in in_features:
                    if not abl:
                        dfoob[(oob_ind[ind],feature_ind)][0] += (j + weight*normalized_distance)
                    else:
                        dfoob[(oob_ind[ind],feature_ind)][0] += normalized_distance
                    dfoob[(oob_ind[ind],feature_ind)][1] += 1
        for key in dfoob:
            dfoob[key] = dfoob[key][0]/dfoob[key][1] if dfoob[key][1]!=0 else 0
